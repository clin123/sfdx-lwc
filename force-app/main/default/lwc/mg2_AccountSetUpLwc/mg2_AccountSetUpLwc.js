...
const columns = [
  ...
  { label: "LAST MODIFIED DATW", fieldName: "LastModifiedDate", type: "date", typeAttributes: { day: 'numeric',
                                                                                                month: 'short',
                                                                                                year: 'numeric',
                                                                                                hour: '2-digit',
                                                                                                minute: '2-digit',
                                                                                                second: '2-digit',
                                                                                                hour12: true
                                                                                              }, sortable: true, cellAttribute: { alignment: "left" } },
...
export default class AcctSetUpLWC extends NavigationMixin(LightningElement) {
  ...
  @track errorPage = false; //boolean to redner wizard page
  ...
  renderPDF() {
    let url = window.location.origin;
    let selectString = this.allList.join();
    let vfURL = url + "/apex/Mg2_NASFView?aid=" + this.recordId + "&renderAs=PDF" + "&Mbr_Group__c=" + selectString;
    window.open(vfURL);
  }
